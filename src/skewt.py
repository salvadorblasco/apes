...
class skewnorm_gen(rv_continuous):
    ...
    def _tInt(h, a, jmax, cutPoint):
        ...
        if hH.size > 0:
            seriesH = (np.arctan(a) * np.exp(-0.5 * (hH ** 2.0) * a / np.arctan(a)) 
                        * (1 + 0.00868 * (hH ** 4.0) * a ** 4.0) / (2.0 * np.pi))
    ...
    def _cumulantsHalfNorm(n=4):
        if skewnorm_gen._cumulantsHalfNormCache.size < n:
            n = max(n, 2)
            n = int(2 * math.ceil(n / 2.0))
            halfN = n / 2
            m = np.arange(0, halfN)
            signs = np.array([1.0, -1.0] * halfN)[0:halfN]
            a = np.zeros(2 * halfN)
            a[2 * np.arange(0, halfN)] = signs * math.sqrt(2.0 / math.pi) / (gamma(m + 1) * 2 ** m * (2 * m + 1))
...
skewnorm = skewnorm_gen(name="skewnorm", shapes="shape")
